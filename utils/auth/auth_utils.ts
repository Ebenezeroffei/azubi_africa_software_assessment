...
import UserRoles, { UserRolesKeyValueSwap } from "@constants/auth/user_roles"
    ...
    static login = async (
        ...
        if (username && Validators.Password.test(password)) {
            ...
            if (res?.ok) {
                ...
                session?.role === UserRolesKeyValueSwap.Manager
                    ? router.push('/dashboard/reports/orders')
    ...
    static validateOTP = async (
        ...
        if (Validators.OTP.test(otp)) {
            ...
            try {
                ...
                await authApi.post('/password-reset/validate-otp/', data)
    ...
    static resetPassword = async (
        ...
        if (Validators.Password.test(newPassword1) && newPassword1 === newPassword2 && Validators.OTP.test(otp)) {
            ...
            try {
                ...
                await authApi.post('/password-reset/confirm/', data)
